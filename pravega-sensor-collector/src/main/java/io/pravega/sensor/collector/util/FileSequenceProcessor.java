...
import io.pravega.sensor.collector.eventgenerator.EventGenerator;
import io.pravega.sensor.collector.eventgenerator.FileEventGenerator;
import io.pravega.sensor.collector.eventgenerator.ParquetEventGenerator;
import io.pravega.sensor.collector.eventgenerator.RawEventGenerator;
...
public class FileSequenceProcessor {
    private static final Logger log = LoggerFactory.getLogger(FileSequenceProcessor.class);

    private final FileSequenceConfig config;
    private final TransactionStateSQLiteImpl state;
    private final EventWriter<byte[]> writer;
    private final TransactionCoordinator transactionCoordinator;
    private final EventGenerator eventGenerator;

    public FileSequenceProcessor(FileSequenceConfig config, TransactionStateSQLiteImpl state, EventWriter<byte[]> writer, TransactionCoordinator transactionCoordinator, EventGenerator eventGenerator) {
        this.config = config;
        this.state = state;
        this.writer = writer;
    ...
    public static FileSequenceProcessor create(
            FileSequenceConfig config, EventStreamClientFactory clientFactory){
        ...
        if(config.fileType.equalsIgnoreCase("PARQUET")){
            eventGenerator = ParquetEventGenerator.create(
                    config.routingKey,
                    config.maxRecordsPerEvent,
                    config.eventTemplateStr,
                    writerId);
        }else if(config.fileType.equalsIgnoreCase("CSV")){
            eventGenerator = FileEventGenerator.create(
                    config.routingKey,
                    config.maxRecordsPerEvent,
                    config.eventTemplateStr,
                    writerId);
        }else{
            eventGenerator = RawEventGenerator.create(
                    config.routingKey,
                    config.eventTemplateStr,
                    writerId);
        }
        ...
        return new FileSequenceProcessor(config, state, writer, transactionCoordinator, eventGenerator);
    }
